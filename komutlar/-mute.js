...
const mutelog = message.guild.channels.cache.find(c => c.id === '792533402917404702')//Mute logu
const muterol = message.guild.roles.cache.find(r => r.id === '790156526769799208')//Muteli rolü
...
data.set(`muteli_${member.guild.id + member.id}`, 'Chat Muted')
data.set(`süre_${member.id + member.guild.id}`, zamandilimi)
                 
message.react('✅')          
message.channel.send(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true}))
                     .setColor('0x348f36')
